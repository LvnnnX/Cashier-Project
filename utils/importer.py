...
def getAmbilById(idAmbil, date: datetime.datetime):
    ...
    ambil["tanggal_stok"] = ambil["tanggal_stok"].apply(
        lambda x: datetime.datetime.strptime(x, stringStrip)
    )
...
def getPossibleStokResolver(idKopi, jumlahPesanan, listOfQuery: list):
    ...
    for z in listOfQueryCopy:
        if len(pastStok.loc[pastStok["id_stok"] == z[0]]) > 0:
            pastStok.loc[pastStok["id_stok"] == z[0], "stok"] = (
                pastStok.loc[pastStok["id_stok"] == z[0], "stok"] + z[1]
            )
            removeList.append(z)
    for k in removeList:
    ...
    if len(pastStok.loc[pastStok["stok_cum"] >= jumlahPesanan]) > 0:
        ...
        returnedRows = pastStok.loc[pastStok["stok_cum"] <= minValue].drop(
            ["stok_cum"], axis=1
        )
        queryOfChange = []
    ...
    for i in range(length_month - 1, -1, -1):
        ...
        for z in listOfQueryCopy:
            if len(stokCsv.loc[stokCsv["id_stok"] == z[0]]) > 0:
                stokCsv.loc[stokCsv["id_stok"] == z[0], "stok"] = (
                    stokCsv.loc[stokCsv["id_stok"] == z[0], "stok"] + z[1]
                )
                removeList.append(z)
        for k in removeList:
            listOfQueryCopy.remove(k)
        ...
        pastStok["stok_cum"] = stok
        if len(pastStok.loc[pastStok["stok_cum"] >= jumlahPesanan]) > 0:
            returnedRows = pastStok.loc[pastStok["stok_cum"] >= jumlahPesanan].drop(
                ["stok_cum"], axis=1
            )
            queryOfChange = []
            for index, row in returnedRows.iterrows():
...
def updateNotaDetailAmbilByIdMultiple(listOfQuery, date: datetime.datetime):
    ...
    Month = HASHMONTH[int(date.strftime("%m")) - 1]
    Day = date.strftime("%d")
    ambilCsv = pd.read_csv(
        DATAPATH / "Nota" / Year / Month / "nota_detail_pulang.csv"
    ).set_index("id_nota_detail")
    for i in listOfQuery:
        ambilCsv["harga_satuan"][i[0]] = i[2]
        ambilCsv["stok"][i[0]] = i[1]
    ambilCsv.reset_index(inplace=True)
    ambilCsv.to_csv(
        DATAPATH / "Nota" / Year / Month / "nota_detail_pulang.csv", index=False
    )
...
def updateStokAvailableByIdMultiple(listOfQuery):
    ...
    dictionaryCsv = {}
    for i in monthPath:
        dictionaryCsv[i] = pd.read_csv(DATAPATH / "Coffee" / i / "stok.csv").set_index(
            "id_stok"
        )
    for query in listOfQuery:
        dictionaryCsv[query[2]]["stok"][query[0]] = (
            query[1] + dictionaryCsv[query[2]]["stok"][query[0]]
        )
    for i in monthPath:
...
def getLatestStokIdByKopiId(IdKopi):
    stokCsv = pd.read_csv(DATAPATH / "Coffee" / "stok.csv")
    stokFilteredSort = (
        stokCsv.loc[stokCsv["id_kopi"] == int(IdKopi)]
        .sort_values(by="tanggal_exp", ascending=False)
        .reset_index()
    )
