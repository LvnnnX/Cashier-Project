...
def main(view: View, super_page: Page):
        ...
        content = Container(
            content=Column(
                controls=[
                    ...
                    Container(PopUp_JumlahBarangField),
                    Container(
                        Text(
                            "Masukkan Diskon (%)",
                            size=16,
    ...
    def PopUp_HapusNota_Card(e):
        return AlertDialog(
            ...
            content=Text(
                ...
                spans=[
                    TextSpan(
                        text="Nota yang sudah dihapus tidak bisa dikembalikan",
                        style=TextStyle(color=colors['Black'], weight="bold")
                    )
                ],
            ),
            actions=alertYaorBatalkan,
        ...
        columns=[
            ...
            DataColumn(
                Text(
                    "Kuantitas",
                    size=12,
                    width=super_page.window_width / 100 * 40 / 100 * 16,
                    color=colors["Gray/400"],
                )
            ),
            DataColumn(
                Text(
                    "Diskon",
                    size=12,
                    width=super_page.window_width / 100 * 40 / 100 * 14,
                    color=colors["Gray/400"],
                )
            ),
            DataColumn(
                Text(
                    "Harga",
                    size=12,
                    width=super_page.window_width / 100 * 40 / 100 * 18,
                    color=colors["Gray/400"],
                )
            ),
            DataColumn(
    ...
    def createNotaRow(data, icon_flag, tempList=[], total_harga=0):
        ...
        if tempList != []:
            ...
            for i in tempList:
                ...
                if barang == str(data[0]["Barang"]) and discount == str(
                    data[0]["Diskon"]
                ):
        ...
        if not flag_stop:
            for num, i in enumerate(data):
                ...
                icon_cells = [
                    ...
                    IconButton(
                        icon=icons.EDIT, disabled=icon_flag, on_click=temp.alert
                    ),
                ]
                ...
                if flag_add:
                    total_harga += (
                        i["Harga"] * i["Kuantitas"] * (1 - (i["Diskon"] / 100))
                    )
        tempList.append(
            DataRow(
                cells=[
                    ...
                    DataCell(
                        Text(
                            "Rp{}".format(changeCurrency(total_harga)),
                            width=rowWidth * 24,
                        )
                    ),
                    DataCell(Text("")),
    ...
    rowDesc = [
        Text("Detail Nota", weight=FontWeight.BOLD, size=20, color=colors["Black"]),
        Container(
            content=Row(
                controls=[
                    Text(
                        "ID Nota : {}".format("id_nota"), color=colors["blackAlpha/500"]
                    ),
                    Text(
                        "Kasir : {}".format("id_kasir"), color=colors["blackAlpha/500"]
                    ),
                ],
                alignment=MainAxisAlignment.SPACE_BETWEEN,
            ),
    ...
    detail_nota_button = Container(
        content=Row(
            controls=detailNotaButton, alignment=MainAxisAlignment.CENTER, spacing=5
        ),
        width=super_page.window_width / 100 * 40,
    )
